  ...
  nh_private_(nh_private)
{
    ...
    if (!nh_private_.getParam ("unknown_value", DSP_UNKNOWN))
        DSP_UNKNOWN = 10000;
    if (!nh_private_.getParam ("path_to_file", path_to_file))
        path_to_file = "path";
...
{
    gdsl_->Plan(path_);
   ROS_INFO("path cost: %f", path_.cost);
...
nav_msgs::Path Dsp::dspPathToRosMsg(const std::vector<Eigen::Vector3d> &dsp_path, bool isSplined)
{
  ...
  std::ofstream myfile;
  //newfile = ("/home/yifan/catkin_ws/src/Dsp/generated_path"+"/"+path_to_file+".txt").c_str();
  //myfile.open ("/home/yifan/catkin_ws/src/Dsp/generated_path/path.txt");
  //myfile.open(newfile);
  myfile.open ((std::string("/home/yifan/catkin_ws/src/Dsp/generated_path_sharepose/")+path_to_file+".txt").c_str());
  for(int i = 0; i < dsp_path.size(); i++)
  {
   ...
   myfile << msg.poses[i].pose.position.x << " " << msg.poses[i].pose.position.y << " "<< msg.poses[i].pose.position.z << "\n";
  }
  myfile.close();
  return msg; 
}
